...
{
	int g_debug = 0;
	int g_color[MAX_COLORS]; // not assigned
	int g_color_cnt = 0;
	...
	printf("orig_ws: %d  mlp: %d\n", orig_ws, mlp);
  	...
  	int memchunk[8192]; //should be 32KB
    ...
    size_t vecSize = 252;
	//std::random_shuffle(myvector, myvector + vecSize ); removing this for now as we only have one color
