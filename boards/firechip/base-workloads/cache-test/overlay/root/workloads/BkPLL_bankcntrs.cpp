...
volatile bool g_should_quit = false;
...
static const uint64_t k_bank_counter_regs[] = {
	0x21000670ULL,
	0x21000678ULL,
	0x21000680ULL,
	0x21000688ULL,
...
static void dump_bank_counter_regs(const char* label)
{
	printf("Bank counter MMIO snapshot (%s)\n", label);
	for (size_t idx = 0; idx < sizeof(k_bank_counter_regs) / sizeof(k_bank_counter_regs[0]); ++idx) {
		char cmd[64];
		snprintf(cmd, sizeof(cmd), "devmem 0x%08" PRIX64, k_bank_counter_regs[idx]);
		int ret = system(cmd);
		if (ret != 0) {
			fprintf(stderr, "devmem failed for 0x%08" PRIX64 "\n", k_bank_counter_regs[idx]);
		}
	}
}
...
int64_t run(int64_t iter, int mlp)
{
	...
	for (int64_t i = 0; i < iter && !g_should_quit; i++) {
		switch (mlp) {
		case 32:
...
long run_write(long iter, int mlp)
{
	...
	for (long i = 0; i < iter && !g_should_quit; i++) {
		switch (mlp) {
		case 32:
...
int main(int argc, char* argv[])
{
	...
	attacker_setup();
	if (system("devmem 0x21000650 64 0") != 0) {
		fprintf(stderr, "devmem failed for 0x21000650 (clear)\n");
	}
	if (system("devmem 0x21000650 64 1") != 0) {
		fprintf(stderr, "devmem failed for 0x21000650 (set)\n");
	}
	dump_bank_counter_regs("before_run");
	...
	dump_bank_counter_regs("after_run");
